...
This guide will show you the basics of creating a simple Velaptor **Hello World** application.
...
to easily create a minimal Velaptor application.
...
Add the Velaptor [NuGet package](https://www.nuget.org/packages/KinsonDigital.Velaptor) to the project.
   ...
   :::info Important
   Velaptor is currently in preview. This is required to see the NuGet package.
   :::
...
But we only need to load a single font for this simple hello Velaptor application.  Create a method override of the `Window.Load()` method
...
A game cannot be a game without adding interactivity and updating something to make things happen.  Otherwise, you are just looking at a still picture!
All games have 2 pieces that make up the game loop.  One piece is the ability to update everything in your game before you render it to the screen.  The other piece
is rendering everything to the screen.  Let's add something to give the application some life.
