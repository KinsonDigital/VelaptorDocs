...
export class VersionsFileService {
    public resetVersionsFile(filePath: string): void {
        if (filePath === undefined || filePath === "") {
            throw new Error("The 'filePath' parameter must not be null or empty.");
        }

        if (extname(filePath) != ".json") {
            throw new Error(`The versions file '${filePath}' must be a '.json' file.`);
        }

        const testingVersion = "1.0.0-testing";
        const fileContents: string = Deno.readTextFileSync(filePath);
        ...
        if (versions.indexOf(testingVersion) === -1) {
            ...
            const fileDataToWrite: string = JSON.stringify(versions, null, 2);

            Deno.writeTextFileSync(filePath, fileDataToWrite);
        }
    }

    public clearTestingVersion(filePath: string): void {
        if (filePath === undefined || filePath === "") {
            throw new Error("The 'filePath' parameter must not be null or empty.");
        }
