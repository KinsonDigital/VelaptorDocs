...
### <span class="color-sub-step">Step 3.1: Create class fields</span>

Create a couple class fields at the top of the class. These will be used to load the font for rendering text.
...
We want to keep the font loader around as a class field so we can unload the font later when shutting down the game.
...
font renderer. To achieve this, you can add the following code to the `OnDraw()` method.
...
We will go over what this is used in the next step.
...
With this information, we can then use it to compare against the sides of the window.  If a collision occurs, we just flip the sign of the `X` and/or `Y` velocity
by negating it. Negating the velocity is what changes the direction of the text's movement.  This is just vector math.
