...
Go to the <TutorialLink projectName="AtlasTextures"/> tutorial project to see a fully working example.
:::
...
of a particular format such as JSON or XML, that describes the location, size, and other kinds of metadata for each image in the atlas texture.
...
and the associated metadata. At this point, it is very easy to automate the process of switching between textures to run an animation. Velaptor comes with loading
atlas textures and their associated metadata built in.
...
Various tools out there give you the ability to easily create atlas textures and metadata. A couple of examples are: [Texture Packer](https://www.codeandweb.com/texturepacker)
...
Plans are in the works to add _**Texture Packer**_ integration. One option is you could create a tool to
load the _**Texture Packer**_ JSON data for use.
:::
...
:::note Project Templates
To see a complete working sample project of this tutorial, look at the <a target="_blank" rel="noopener" class="link" href="https://github.com/KinsonDigital/VelaptorTutorials/tree/master/AtlasTextures">AtlasTextures<OpenNewTab style={{paddingTop: '10', paddingLeft: '0.25%', width: '2%'}}/></a>
&nbsp;project in the <a target="_blank" rel="noopener" class="link" href="https://github.com/KinsonDigital/VelaptorTutorials/tree/master/VelaptorTutorials">
VelaptorTutorials<OpenNewTab style={{paddingTop: '10', paddingLeft: '0.25%', width: '2%'}}/></a> repository.
:::
...
Add the downloaded atlas texture and metadata JSON file to the _**Content/Atlas/**_ directory in the project.
:::note REFER TO
...
```
The `atlasData` variable will contain the loaded texture and the metadata for the texture.
