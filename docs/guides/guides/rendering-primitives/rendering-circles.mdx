...
for precision. You can also use the `Diameter` property to set the size of the circle.
...
If you were to move the circle to the right by **30** pixels to the right using the `Left` property, then
...
Here is an example of how to create a solid circle:
