...
The first sentence could be improved by adding a subject. For example: "The wonder of interactivity lies in being able
to control your game with a keyboard or other types of hardware input.  Without the ability to control your game, you
are just staring at a looping animation.  The "interface" between the human playing the game and the game responding
to the human is the hardware such as a keyboard or mouse.
...
The only game loop methods required for this tutorial will be the `OnUpdate()` method, which we
...
You can create a new keyboard object quickly and easily using Velaptor's 'InputFactory' class.
