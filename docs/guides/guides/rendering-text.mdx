...
Go to the <GuideLink projectName="RenderingText" /> guide project to see the source code for a fully working example of this guide.
:::
...
```csharp
private IContentManager contentManager; // Loads various types of content
...
Set the window title and size in the `Game` constructor.  This is self explanatory.
...
Now we can create the content manager, font renderer, and a batcher.  These will be used to
...
What we are doing is making **Velaptor** aware that we will be rendering a bunch of textures all at once.  By rendering textures **together**, we mean
...
<NumberedList items={[
    <>Invoke <code>IBatcher.Begin()</code> method</>,
    <>Invoke all <code>Render()</code> methods for the current frame</>,
    <>Invoke the <code>IBatcher.End()</code> method</>
]} />
...
We need to load the font and to do this we can use the content manager created earlier.
This can be created using the `ContentManager.Create()` method.

Add the following code to the `OnLoad()` method to load the font.
...
omit the file extension when using the `this.contentManager.LoadFont()` method. Also, the name is not _**case-sensitive**_.  
...
and send in the `IFont` class field we created earlier.

Let's override the `OnUnload()` method and unload the font.  It is always a good idea to unload
...
:::tip Content Caching
When you load content, it is cached in memory.  This means that if you load the same content multiple times,
it will only be loaded once and the cached version will be returned for subsequent loads.
This is a good thing because it helps to improve performance and reduce memory usage.

If you want to take advantage of this caching mechanism in the next scene, do not unload the content in the `OnUnload()` method.
:::
