...
Go to the <TutorialLink projectName="MouseInput"/> tutorial project to see a fully working example.
:::
...
Create some class fields that can hold the state of the mouse and also the amount of how much we want to change
...
You can create a new mouse object quickly and easily using Velaptor's `InputFactory` class.
...
Next, between the two mouse states in the `OnUpdate()` method, add the code below to change the width of the window.
...
This code checks if the left mouse button is currently being held down. If it is, then the width of the window will be
changed based on the difference between the current and previous mouse positions.  As you move the mouse left and right,
it will take the difference between the position on the X-axis and apply that to the width of the window.
...
Add the following code right below the `if` block added in the previous step.
...
This code will get the current value of the mouse wheel and use this to detect which direction the mouse wheel was scrolled.
When the mouse wheel is scrolled down toward you, it returns a negative value and when it is scrolled up away from you,
...
It would be nice to see the current size of the window. To do this, we can override the `OnResize()` method
