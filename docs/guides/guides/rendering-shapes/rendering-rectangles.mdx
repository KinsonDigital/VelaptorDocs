...
The border thickness is controlled by the `BorderThickness` property and is of type `float`.
...
You can easily check if the rectangle contains a `Vector2` in 2D space.  This can be done by using
the `Contains()` method.
