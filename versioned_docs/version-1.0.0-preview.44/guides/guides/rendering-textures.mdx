...
Create the class fields shown below. These will be used to render the Velaptor logo texture to the screen.
...
public class Game : Window
{
    ...
    private IContentManager contentManager; // Used to load content
    private ITexture? logoTexture; // The texture to render
    ...
}
```
...
### <span className="color-sub-step">2.3: Create content manager, batcher, & renderer</span>

In the constructor, create the `contentManager`, `batcher`, and `textureRenderer` objects.
...
when it is not needed or used anymore.  In the next couple of steps, we will setup the loading and unloading of a texture.

### <span className="color-sub-step">3.1: Get the content</span>

Download or create any _**.png**_ file you want.

:::info Velaptor Logo Image
Click the image below to download the file.

<StaticDownload relativePath="/img/velaptor-logo.png" sizePercentage={12}/>
:::
...
We can load the texture by using the `contentManager`.  There are various loaders available in **Velaptor**
...
and send in the texture object we loaded earlier.

Let's override the `OnUnload()` method and unload the texture.  It is always a good idea to unload
