...
To see a complete working sample project of this tutorial, look at the [HelloVelaptor](https://github.com/KinsonDigital/VelaptorTutorials/tree/master/HelloVelaptor) project in the [VelaptorTutorials](https://github.com/KinsonDigital/VelaptorTutorials) repository.
:::

### <span class="color-step">Step 1: Loading content</span>



#### <span class="color-sub-step">Step 1.1</span>

Create a class field named `font` at the top of the class.  This will the loaded font to use for rendering text.
...
### <span class="color-step">Step 2: Rendering text</span>
...
Now that we have a renderer and a font to render text to the screen we need to use the renderer to render some text to the screen using our font.
Add the code below to the `OnDraw()` method to accomplish this.  Note that we need to invoke `IRenderer.Begin()` before we invoke any `Render()` methods
for any renderers and then invoke the `IRenderer.End()` when we are finished with all of our rendering.

:::info
What is this render begin and end thing?

<details><summary>Expand me to learn more!</summary>

This has to do with batching.  When we call `Begin()`, we are telling Velaptor that we are ready to start _**'batching'**_ all of our textures together.
As we call any `Render()` method for any renderer types, we are adding textures to the _**'batch'**_.  Once we invoke the `End()` method,
this tells Velaptor that we are done batching all of our textures.
Once we invoked the `End()` method, this is when the actual rendering occurs.

Just remember the steps:
1. Invoke `IRenderer.Begin()` method
2. Invoked all `Render()` methods for the current frame
...
### <span class="color-step">Step 6: Let's update something</span>

A game cannot be a game without adding interactivity to make things happen.  Otherwise, you are just looking at a still picture!
...
is rendering the screen.  Let's add something to give the application some life.
